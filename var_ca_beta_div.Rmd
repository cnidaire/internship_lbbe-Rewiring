...
abstract: >
  The goal is to look at the variance of the positions in the Correspondance Analysis that we think is a proxy of the ability of one specie to do rewiring. And the corruent way of quantifying the individual contribution to rewiring which was defined in Toju et al (2024), consisting in using the metrics of beta diversity defined by poisot and assume that by removing the specie and computing again the Beta diversities, we can obtain the contribution of the specie.
...
```{r}
simu_network <- env_grad_netw(nb_resource = 50, nb_consumer = 50, nb_location = 2,
              ninter = 40000,
...
this array is in the following structure: array_3D[res, con, frame]
...
mean(betalinkr_multi(array_3d[,,], partitioning="poisot", binary=FALSE)[,"OS"])
mean(betalinkr_multi(array_3d[-1, , ], partitioning="poisot", binary=FALSE)[,"OS"])
# array_3d[,,-2]
```

```{r}
apply(array_3d[,,1], 1, betalinkr)
```
```{r}
lapply(simu_network$obs_network)
betalinkr(simu_network$obs_network[[1]], tab)
...
```{r}
# tab
betalinkr(abind(array_3d[-1,,1], tab[-1,], along = 3))["OS"]
betalinkr(abind(array_3d[,,1], tab[,], along = 3))["OS"]
```

```{r}
beta_indiv_res <- array(data = NA, 
                    dim = c(nrow(simu_network$obs_network[[1]]), dim(array_3d)[3]))

rownames(beta_indiv_res) = rownames(simu_network$obs_network[[1]])
colnames(beta_indiv_res) = dimnames(array_3d)[[3]]

# generate empty array with frame in column and resources in row, it will contain the beta_os individual contribution compared to the meta-network. by making the difference when removing one specie in both networks.

beta_os <- function(x){
  betalinkr(abind(array_3d[-x,,f], tab[-x,], along = 3))[["OS"]]
}

input_values <- 1:nrow(simu_network$obs_network[[1]])

for (f in 1:dim(array_3d)[3]) {
  beta_os_netw <- betalinkr(abind(array_3d[,,f], tab[,], along = 3))[["OS"]]
  # result <- beta_os_netw - mclapply(input_values, beta_os, mc.cores = 4)
  result <- beta_os_netw - unlist(mclapply(input_values, beta_os, mc.cores = 4))
  beta_indiv_res[,f] <- result
}
beta_indiv_res
sum(beta_indiv_res[,1])
```
...
mean_beta_os <- mean(betalinkr_multi(array_3d, partitioning="poisot", binary=FALSE)[, "OS"])

my_function <- function(x) {
  # mean(betalinkr_multi(aperm(array_3d, c(2,3,1)), partitioning="poisot", binary=FALSE)[-x, "OS"])
  mean_beta_os - mean(betalinkr_multi(array_3d[-x,,], partitioning="poisot", binary=FALSE)[, "OS"])
}
