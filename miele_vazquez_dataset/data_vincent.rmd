...
table.value(distrib_int$sd, grid = F) # plot of SD
table.value(distrib_int$mean, grid = F) # plot of Mean
...
table.value(distrib_variation) # SD/Mean
# distrib_int$sd != 0 & distrib_int$mean != 0
```

```{r}
hist(log(distrib_variation[which(distrib_variation != 0)]), main = "log(SD/Mean)")
boxplot(as.vector(distrib_variation[which(distrib_variation != 0)]), main = "SD/Mean")
hist(as.vector(distrib_variation[which(distrib_variation != 0)]), main = "SD/Mean")
```
...
dimnames(neutral_effect)[1] <- dimnames(bipartite.networks)[1] # to have the time frame names
```
...
trait_effect <- aperm(trait_effect, c(3, 1, 2)) # same as previously
...
var_trait <- function(spe_1, spe_2){
  int_frame <- names(which(neutral_effect[,spe_1, spe_2] > 0)) # which frame do we have coexistence of species
  if (length(int_frame) >= 3){
    pot_int <- trait_effect[int_frame, spe_1, spe_2]
    return(list(mean = mean(pot_int), sd = sd(pot_int)))
  }
  else {
    return(list(mean = 0, sd = 0)) # if not enough coexistence frames
  }
}
...
hist(as.vector(distrib_trait$sd/distrib_trait$mean), main = "SD/Mean", xlab = "SD/Mean") # as NaN are not displayed (when 0/0), we do not have to worry about the 0
hist(as.vector(distrib_trait$mean)[which(as.vector(distrib_trait$mean) != 0)], main = "Mean distribution", xlab = "Mean")
hist(as.vector(distrib_trait$sd)[which(as.vector(distrib_trait$sd) != 0)], main = "SD distribution", xlab = "SD")

plot(as.vector(distrib_trait$mean)[which(as.vector(distrib_trait$mean) != 0)], as.vector(distrib_trait$sd)[which(as.vector(distrib_trait$sd) != 0)], main = "SD & Mean", xlab = "Mean", ylab = "SD")
```

### Same without the neutral effect

only plot the number of interactions from bipartite.network

```{r}
barplot((table(as.vector(bipartite.networks)[as.vector(bipartite.networks != 0)] )), main = "Distribution of non 0 interactions accross the network", ylab = "number of interacting species couple", xlab = "number of interactions")

# table(as.vector(bipartite.networks)[as.vector(bipartite.networks != 0)] )
```

```{r}
bipartite_mean <- apply(bipartite.networks, c(2,3), mean)
bipartite_sd <- apply(bipartite.networks, c(2,3), sd)
```

```{r}
barplot(table(as.vector(bipartite_sd/bipartite_mean)), main = "SD/Mean", xlab = "SD/Mean") # as NaN are not displayed (when 0/0), we do not have to worry about the 0

# table(as.vector(bipartite_sd/bipartite_mean))

barplot(table(as.vector(bipartite_mean)[which(as.vector(bipartite_mean) != 0)]), main = "Mean number of interaction's distribution", ylab = "Number of interacting species couple", xlab = "Mean")

table(as.vector(bipartite_mean)[which(as.vector(bipartite_mean) != 0)])

barplot(table(as.vector(bipartite_sd)[which(as.vector(bipartite_sd) != 0)]), main = "SD of the number of interaction's distribution accross the network", ylab = "Number of interacting species couple", xlab = "SD")

# table(as.vector(bipartite_sd)[which(as.vector(bipartite_sd) != 0)])

plot(as.vector(bipartite_mean)[which(as.vector(bipartite_mean) != 0)], as.vector(bipartite_sd)[which(as.vector(bipartite_sd) != 0)], main = "SD & Mean", xlab = "Mean", ylab = "SD")

abline( lm(as.vector(bipartite_sd)[which(as.vector(bipartite_sd) != 0)]~as.vector(bipartite_mean)[which(as.vector(bipartite_mean) != 0)]), col = "red")
```

### Interaction VS neutral effect

```{r}
plot(bipartite.networks, neutral_effect, xlab = "number of interactions", ylab = "neutral effect", main = "Interaction VS Neutral effect")

abline(lm(as.vector(neutral_effect) ~ as.vector(bipartite.networks)), col = "red")

conf_int <- confint(lm(as.vector(neutral_effect) ~ as.vector(bipartite.networks)))

abline(conf_int[,1], col = "blue")
abline(conf_int[,2], col = "blue")

plot(log(bipartite.networks), log(neutral_effect), xlab = "number of interactions", ylab = "neutral effect", main = "Interaction VS Neutral effect \n double log scale")
```

```{r}
cor(as.vector(bipartite.networks), as.vector(neutral_effect), method = "spearman")
cor(as.vector(bipartite.networks), as.vector(neutral_effect), method = "pearson")
```

We can see that there is many interactions that are not linearly correlated with the neutral effect. It means that these couple of species are interacting more or less than what would be expected if the interaction were only driven by the neutral effects. It is indeed related as the correlation coefficient is of 0.62 but it also depends on the trait matching and a good trait matching can increase the interaction probability and reciprocally.

```{r}
lm(as.vector(neutral_effect) ~ as.vector(bipartite.networks))
confint(lm(as.vector(neutral_effect) ~ as.vector(bipartite.networks)), level = 0.95)
```


The equation of the linear regression is $y = 822x+165$

let's look at the interactions that have more than 500,000 as a neutral effect score and less than 500 Interactions

```{r}
(out_layer <- which(neutral_effect > 500000 & bipartite.networks <500, arr.ind = T))
```

We don't care about the dimension 1 as it is the time frame

```{r}
interest_spe <- list(pla = array(NA, dim = dim(out_layer)[1]), pol = array(NA, dim = dim(out_layer)[1]))
for (i in 1:dim(out_layer)[1]){
  interest_spe$pla[i] <- dimnames(bipartite.networks)[[2]][out_layer[i, 2]]
  interest_spe$pol[i] <- dimnames(bipartite.networks)[[3]][out_layer[i, 3]]
}
interest_spe$pla
interest_spe$pol
```
