...
```{r}
neutral_effect <- lapply(1:18, function(i) outer(rowSums(bipartite.networks[1,,]), colSums(bipartite.networks[1,,]))) #nearly the same as making a matrix product between the two sum vectors but here with the array format I keep the names

trait_effect <- lapply(1:18, function(i) bipartite.networks[1,,] / neutral_effect[[1]])
```

```{r}
names(lapply(1:18, function(i) which(neutral_effect[[i]]["con.mic", "bombil11"] != 0)))
```



```{r}
neutral_effect <- sapply(1:18, function(i) outer(rowSums(bipartite.networks[1,,]), colSums(bipartite.networks[1,,])), simplify = "array") #nearly the same as making a matrix product between the two sum vectors but here with the array format I keep the names

str(neutral_effect)

neutral_effect <- aperm(neutral_effect, c(3,1,2))

dimnames(neutral_effect)[1] <- dimnames(bipartite.networks)[1]

str(neutral_effect)

str(bipartite.networks)

class(neutral_effect)
```
...
var_trait <- function(spe_1, spe_2){
  int_frame <- names(which(neutral_effect[[1]][spe_1, spe_2] != 0)) # get only the frames where there is interaction
  if (length(int_frame) >= 2) { # if more than 3 frames
    return(list(mean = mean(trait_effect[[1]][spe_1, spe_2]), sd = sd(trait_effect[[1]][spe_1, spe_2])))
  } 
  else {
    return(list(mean = 0, sd = 0)) # if not enough int observed in the network serie
  }
...
var_trait <- function(spe_1, spe_2){
  int_frame <- names(lapply(1:18, function(i) which(neutral_effect[[i]][spe_1, spe_2] != 0))) # get only the frames where there is interaction
  if (length(int_frame) >= 3) { # if more than 3 frames
    return(list(mean = mean(lapply(1:18, function(i) trait_effect[[i]][spe_1, spe_2])), 
                sd = sd(lapply(1:18, function(i) trait_effect[[i]][spe_1, spe_2]))
                ))
  } 
  else {
    return(list(mean = 0, sd = 0)) # if not enough int observed in the network serie
...
which(distrib_trait$mean != 0)

# table.value(distrib_trait$sd, grid = F)
# table.value(distrib_trait$mean, grid = F)
# 
# distrib_variation <- array(0, dim = c(59, 196))
# rownames(distrib_variation) <- plant.id
# colnames(distrib_variation) <- pollinator.id
...
# 
# 
# # hist(log(as.vector(trait_effect[!is.na(trait_effect)])))
# # 
# # as.vector(trait_effect[!is.na(trait_effect)])
