...
For each Of the points, I'll do it for only the first network
...
var_int <- function(spe_1, spe_2){
  int_frame <- names(which(bipartite.networks[ , spe_1, spe_2] != 0))
  if (length(int_frame) > 3) {
    int_count <- c()
    for (i in int_frame){
      abund_1 <- sum(bipartite.networks[i, spe_1, ])
...
# var_int("con.mic", "bombil11")
# 
# names(which(bipartite.networks[ , 32, "bombil11"] != 0))
