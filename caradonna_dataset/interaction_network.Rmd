...
library(network)
library(sna)
ggplot(ggnetwork(data1), aes(x = x, y = y, xend = xend, yend = yend)) + 
  geom_edges() +
  geom_nodes(color = "gold", size = 10) +
  theme_blank()
...
```{r}
data1 <- sub_set(interaction_netw, c(2013), c(1 :4), c("shady_point","twin_meadows"))
graph <- graph_from_data_frame(data1, directed = T)

# Assign colors to nodes based on column
V(graph)$color <- ifelse(V(graph)$name %in% data1$pollinator, "darkgoldenrod1", "chartreuse4")
```
...
plot(graph,
  ...
  # vertex.label = NA,
  vertex.label.dist = 0,  # Distance of label from node
...
# Component Analysis of the full network (average graph)

First of all let's see what does the full networklook like
...
## Make a dataframe containing the frequencies

```{r}
freq_table <- as.data.frame.matrix(table(interaction_netw$plant, interaction_netw$pollinator)) 
```

## Try CA
have to use dudi.coa
...
```{r}
min(table(interaction_netw$plant, interaction_netw$pollinator))
which(is.na(freq_table))
```

```{r}
x <- dudi.coa(freq_table, scannf = T, nf = 2) 
```
