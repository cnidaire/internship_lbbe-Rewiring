...
sub_set <- function(data, year, week, transect){
  return(distinct(
    interaction_netw[data$year == year & data$week_num == week & data$transect == transect, c("pollinator", "plant")]
  ))
}

data1 <- sub_set(interaction_netw, 2013, 2, 1)
```
...
bipartite_layout_edge <- function(edge_list,vertices_position,){
  ...
  edges = left_join(edges_list,vert_pos, by = "name")
