...
scatter(dudi.coa(simulated_network$network[[1]], scannf = F, nf = 2))
```
...
```{r}
# kplot()
k_table_format <- lapply(simulated_network$network, as.data.frame)

class(k_table_format[[1]])

# is.ktab(lapply(dudi.ktable simulated_network$network, ))
?dudi.ktab
# as.data.frame(simulated_network$network[[1]])
