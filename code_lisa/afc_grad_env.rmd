...
simulated_network <- env_grad_netw(nb_resource =spe_resource , nb_consumer = spe_consum, nb_location = nb_frame, ninter = 200)
...
```{r}
plot_matrix(data.frame(simulated_network$th_network[[30]]), 
          max_size = 15) +
...
```{r}
plot_matrix(data.frame(simulated_network$obs_network[[30]]), 
          max_size = 15) +
theme(axis.text = element_blank(),
      axis.text.x.top = element_blank(),
      axis.title = element_blank(),
