...
for (r in 1:nrow(effect_tol_frame)) {
  tr1ax1 <- c()
  tr2ax2 <- c()
  rv_distrib <- c()
  ratio_grad = 0.7
  mean_tol_env = effect_tol_frame$tol_env[r]
  ninter = 5000 / effect_tol_frame$frame[r]
  # ninter = param
  nb_frame = effect_tol_frame$frame[r]
...
for (res in 1:nrow(simu_network$obs_network[[1]])) {
  res_dist[res, "dist"] = sum(dist.binary(array_3d[res,,], method = 1), na.rm = T) # sum of the distances
...
for (con in 1:ncol(simu_network$obs_network[[1]])) {
  con_dist[con,"dist"] = sum(dist.binary(array_3d[,con,], method = 1), na.rm = T) # sum of the distances
...
# Axis 2
ggplot() +
  geom_point(data = as.data.frame(res_dist), aes(x = dist, y = res_var_ax2, colour = obs)) +
  xlab("jaccard dissimilarity") +  # Add X axis label
  ylab("resource variance on axis 1") +  # Add Y axis label
  ggtitle("Jaccard dissimilarity vs variance in axis 2 position for resource") +  # Add plot title
  scale_colour_gradient(low = "cyan", high = "navy", name = "nb obs") +
  theme_bw()
...
end_time <- Sys.time()
end_time - start_time
hist(mean(nw_beta_div$OS) - beta_indiv_res[, 1], breaks = 10)
hist(- beta_indiv_res[, 1], breaks = 10)

boxplot(mean(nw_beta_div$OS) - beta_indiv_res[, 1], breaks = 10)
boxplot(- beta_indiv_res[, 1], breaks = 10)
```

```{r}
sum(mean(nw_beta_div$OS) - beta_indiv_res[, 1])
mean(nw_beta_div$OS)
```
...
boxplot(mean(nw_beta_div$OS) - beta_indiv_con[, 1], main = "distance to beta_os",breaks = br)
boxplot(mean(beta_indiv_con[, 1]) - beta_indiv_con[, 1], main = "distance to mean delta_beta_os", breaks = br)
boxplot(- beta_indiv_con[, 1], main = "contribution to beta_os (delta_beta_os)",breaks = br)
```
