...
```{r}
sum(dist.binary(array_3d[,,1], method = 1)) # sum of the distances
...
```{r}
res_dist <- array(rep(0, ncol(simu_network$obs_network[[1]])*2),
                  dim = c(ncol(simu_network$obs_network[[1]]),2))
colnames(res_dist) <- c("dist", "obs")
rownames(res_dist) <- colnames(simu_network$obs_network[[1]])
# res_dist

# dimnames(res_dist)[1] <- c("dist", "nb_obs")
for (res in 1:ncol(simu_network$obs_network[[1]])) {
  res_dist[res,"dist"] = sum(dist.binary(array_3d[res,,], method = 1), na.rm = T) # sum of the distances
  res_dist[res, "obs"] = sum(array_3d[res,,]) # number of observation for this specie
}
res_dist[,1]
...
```{r}
res_dist <- cbind(res_dist, res_var)
```

mettre tout ce qui est necessaire pour les ressources dans un tableau et de même pour tout ce qui concerne les consommateurs.

```{r}
# In black and white
ggplot() +
  geom_point(data = as.data.frame(res_dist), aes(x = dist, y = res_var, alpha = obs)) +
...
# Axis 1
ggplot() +
  geom_point(data = as.data.frame(res_dist), aes(x = dist, y = res_var, colour = obs)) +
  xlab("jaccard dissimilarity") +  # Add X axis label
  ylab("resource variance on axis 1") +  # Add Y axis label
  ggtitle("Jaccard dissimilarity vs variance in axis 1 position for resource") +  # Add plot title
