...
beta_indiv <- array(data = NA, 
                    dim = c(nrow(simu_network$obs_network[[1]]), 1))

rownames(beta_indiv) = row.names(simu_network$obs_network[[1]])
colnames(beta_indiv) = "delta_os"
# beta_indiv
...
registerDoParallel(cl)
start_time <- Sys.time()
# a <- times(5) %do% Sys.sleep(1)
foreach(i=1:6) %do% Sys.sleep(1)
end_time <- Sys.time()
stopCluster(cl)
end_time - start_time
```
