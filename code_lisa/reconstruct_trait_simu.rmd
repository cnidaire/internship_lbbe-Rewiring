...
degree(graph_from_incidence_matrix(tab[rowSums(tab) != 0, colSums(tab) != 0]))

fou1 = foucart(lapply(simulated_network$obs_giant_compo, as.data.frame), scannf = F, nf = 4)
plot(fou1, yax = 2)
```
...
vert_b_res <- c()
for(a in vert_a_res){
  vert_a_b <- which(tab[a,] !=0)
  vert_b_res <- c(vert_b_res, vert_a_b)
  conn_vert_b_res <- c(conn_vert_b_res, colSums(tab)[names(vert_a_b)])
}
vert_a_res
vert_b_res
conn_vert_b_res
...
```{r}
dim(simulated_network$obs_giant_compo[[1]])
```
