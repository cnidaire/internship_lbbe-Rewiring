...
ggsave(file = "figure_report/env.pdf", plot = figure, width = 8, height = 4)
```
...
ggsave(file = "figure_report/ninter.pdf", plot = figure, width = 8, height = 4)
```
...
```{r}
data <- rbind(
  cbind(effect_tol_frame_1_r, Axis = as.factor("axis 1"), Species = as.factor("Resources")),
  cbind(effect_tol_frame_2_r, Axis = as.factor("axis 2"), Species = as.factor("Resources")),
  cbind(effect_tol_frame_1_c, Axis = as.factor("axis 1"), Species = as.factor("Consumers")),
  cbind(effect_tol_frame_2_c, Axis = as.factor("axis 2"), Species = as.factor("Consumers")))

data

ggplot(data, aes(x = as.factor(frame), y = as.factor(tol_env), fill = mean_corr)) +
  facet_wrap(~ Species + Axis) +
...
```{r}
fig1 <- ggplot(effect_tol_frame_1_r, aes(x = as.factor(frame), y = as.factor(tol_env), fill = mean_corr)) +
  geom_tile() + scale_fill_paletteer_c("grDevices::YlGnBu", direction = -1, limits = c(0, 1)) +
  labs(x = "number of frame",y = "mean tolerance of the env niche") + 
  geom_text(aes(label=round(mean_corr, 2)))

fig2 <- ggplot(effect_tol_frame_2_r, aes(x = as.factor(frame), y = as.factor(tol_env), fill = mean_corr)) + 
  geom_tile() + scale_fill_paletteer_c("grDevices::YlGnBu", direction = -1, limits = c(0, 1)) +
  labs(x = "number of frame",y = "mean tolerance of the env niche") + 
  geom_text(aes(label=round(mean_corr, 2)))
...
ggsave(file = "figure_report/frame_env.pdf", plot = ggarrange(fig1, fig2), width = 8, height = 4)
```
...
ggsave(file = "figure_report/frame.pdf", plot = figure, width = 8, height = 4)
```
...
ggsave(file = "figure_report/delta.pdf", plot = figure, width = 8, height = 4)
```
...
ggsave(file = "figure_report/ratio.pdf", plot = figure, width = 8, height = 4)
```
...
ggsave(file = "figure_report/trait.pdf", plot = figure, width = 8, height = 4)
```
