...
ggsave(file = "figure_report/ninter.pdf", plot = figure, width = 12, height = 8)
```
...
ggsave(file = "env.pdf", plot = ggarrange(fig1, fig2), width = 12, height = 8)

# paletteer_c("ggthemes::Orange", 5)
```
...
```{r}
# avec ggplot
fig1 <- ggplot(gather(param_tr1ax1_l), aes(x = key, y = value)) + 
  geom_boxplot() + 
  ggtitle("Trait 1 Axis 1") +
  ylab("Correlation") +
  xlab("Number of interactions") +
  theme_bw()
fig2 <- ggplot(gather(param_tr2ax2_l), aes(x = key, y = value)) + 
  geom_boxplot() + 
  ggtitle("Trait 2 Axis 2") +
  ylab("Correlation") +
  xlab("Number of interactions") +
  theme_bw()

fig1 <- ggplot(gather(param_tr1ax1_c), aes(x = key, y = value)) + 
  geom_boxplot() + 
  ggtitle("Trait 1 Axis 1") +
  ylab("Correlation") +
  xlab("Number of interactions") +
  theme_bw()
fig2 <- ggplot(gather(param_tr2ax2_c), aes(x = key, y = value)) + 
  geom_boxplot() + 
  ggtitle("Trait 2 Axis 2") +
  ylab("Correlation") +
  xlab("Number of interactions") +
...
ggarrange(fig1, fig2)
ggsave(file = "ninter.pdf", plot = ggarrange(fig1, fig2), width = 12, height = 8)
```
...
for (r in 1:nrow(effect_tol_frame_1)) {
  ...
  ninter = 2500 / effect_tol_frame_1$frame[r]
  # ninter = param
...
ggplot(data, aes(x=axis, y=value, fill=key)) + 
    geom_boxplot() + 
    facet_wrap(~spe) + 
    ylab("Correlation (absolute value)") + 
    xlab("") + 
...
```{r}
fig1 <- ggplot(effect_tol_frame_1, aes(x = as.factor(frame), y = as.factor(tol_env), fill = mean_corr)) + geom_tile() + scale_fill_gradient(low = "white", high = "blue", limits = c(0, 1)) + labs(x = "number of frame",y = "mean tolerance of the env niche") + ggtitle("Trait recovering performance depending on the number of frames and  \n the tolerance of the species toward the environment (axis1)") + geom_text(aes(label=round(mean_corr, 2)))

fig2 <- ggplot(effect_tol_frame_2, aes(x = as.factor(frame), y = as.factor(tol_env), fill = mean_corr)) + geom_tile() + scale_fill_gradient(low = "white", high = "blue", limits = c(0, 1)) + labs(x = "number of frame",y = "mean tolerance of the env niche") + ggtitle("Trait recovering performance depending on the number of frames and \n the tolerance of the species toward the environment (axis2)") + geom_text(aes(label=round(mean_corr, 2)))

ggarrange(fig1, fig2)

ggsave(file = "frame_env.pdf", plot = ggarrange(fig1, fig2), width = 12.5, height = 6)
```
...
ggsave(file = "figure_report/frame.pdf", plot = figure, width = 12, height = 8)
```
...
# ggsave(file = "delta.pdf", plot = ggarrange(fig1, fig2), width = 12, height = 8)
```
...
ggsave(file = "figure_report/delta.pdf", plot = figure, width = 12, height = 8)
```
...
ggsave(file = "figure_report/ratio.pdf", plot = figure, width = 12, height = 8)
```
...
figure <- ggplot(data, aes(x=axis, y=value, fill=key)) + 
    ...
    theme_bw() +
    scale_fill_manual(values = rev(paletteer_c("grDevices::Sunset", 6))) +
    labs(fill = "Trait tolerence")

figure

ggsave(file = "figure_report/trait.pdf", plot = figure, width = 12, height = 8)
```
...
ggplot() +
  ...
  ylab("consumer variance on axis 1") +  # Add Y axis label
  ggtitle("Jaccard dissimilarity vs variance in axis 1 position for consumer") +  # Add plot title
  scale_colour_gradient(low = "#88CCEE", high = "navy", name = "nb obs")
...
res_dist <- cbind(res_dist, bc = NA)
con_dist <- cbind(con_dist, bc = NA)
```

```{r}
vegdist(t(array_3d[200,,]))
for (res in 1:nrow(res_dist)) {
  res_dist[res, "bc"] = sum(vegdist(t(array_3d[res,,])), na.rm = T) # sum of the distances using bray curtis
}

for (con in 1:ncol(simu_network$obs_network[[1]])) {
  con_dist[con,"bc"] = sum(vegdist(t(array_3d[,con,])), na.rm = T) # sum of the distances using bray curtis
}
```
...
```{r}
simu_network <- env_grad_netw(nb_resource = 100, nb_consumer = 100, nb_location = 20,
              ninter = 650,
              mean_tol_env = 0.7, mean_tol = 0.1,
              ratio_grad = 0.5, delta = 0.2)
```
