...
```{r}
(abundance <- abund_env_grad(nb_resource = 2, nb_consumer = 10, nb_location = 10))
```

```{r}
plot(abundance$abundance_consumer[,4])
...
To import the data visualization package of Lisa
...
frame = 60
spe = 10
test_1 <- env_grad_netw(nb_resource =spe , nb_consumer = spe, nb_location = frame)
for (i in 1:frame) {
  # png(file = paste0("test",i, ".png"))
  
  p <- plot_matrix(data.frame(test_1$network[[i]]), 
            max_size = 15) +
...
```{r}
plot(test_1$abudance$abundance_resource[,3])
```

```{r}
plot(test_1$abudance$abundance_consumer[,5])
```

```{r}
apply(test_1$abudance$abundance_resource, 1, min)
apply(test_1$abudance$abundance_resource, 2, min)
test_1$abudance$abundance_resource
```
...
full_abund <- cbind(test_1$abudance$abundance_resource, test_1$abudance$abundance_consumer)

beta_div_abund <- matrix(,nrow = 10, ncol = 10)
for (i in 1:10) {
  for (j in i:10) {
  # for (j in 1:10) {
        c <- apply(full_abund[c(i, j), ], 2, min)
    beta_div_abund[i, j] <- 1- (2*sum(c) / (sum(full_abund[i,]) + sum(full_abund[j, ])))
  }
}

beta_div_abund[beta_div_abund ==0] <- NA
beta_div_abund
```
...
```{r}
library(abind)
# apply(abind(test_1$network[[1]], test_1$network[[2]], along = 3), c(1, 2), min)

beta_div_link <- matrix(,nrow = 10, ncol = 10)
for (i in 1:10) {
  for (j in i:10) {
  # for (j in 1:10) {
    c <- apply(abind(test_1$network[[i]], test_1$network[[j]], along = 3), c(1, 2), min)
    beta_div_link[i, j] <- 1- (2*sum(c) / (sum(test_1$network[[i]]) + sum(test_1$network[[j]])))
  }
}
...
```{r}
# cor(beta_div_abund, beta_div_link, method = "pearson", use = "pairwise.complete.obs")

cor(na.omit(as.vector(beta_div_abund)), na.omit(as.vector(beta_div_link)), method = "pearson")
```

```{r}
# Création de deux matrices d'exemple
matrice1 <- matrix(c(1, 2, 3, 4, 5, 6), nrow = 2)
matrice2 <- matrix(c(6, 5, 4, 3, 2, 1), nrow = 2)

# Calcul de la corrélation de Pearson entre les deux matrices
correlation_pearson <- cor(matrice1, matrice2, method = "pearson")
