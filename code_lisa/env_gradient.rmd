...
spe = 20
sampling_size = 300
test_1 <- env_grad_netw(nb_resource =spe , nb_consumer = spe, nb_location = frame, know_env_grad_pos = F, mean_tol_env = 2,5, sd_tol_env = 1, ninter = sampling_size)
for (i in 1:frame) {
  p <- plot_matrix(data.frame(test_1$obs_network[[i]]), 
            max_size = 15)  +
...
```{r}
network <- cor_abund_grad(frame = 10, spe = 60)
network$correlation
...
```{r}
plot(network$env_grad, network$link, ylim = c(0, 1), ylab = "Beta div link", xlab = "Distance grad env")
```


```{r}
plot(network$abundance, network$link, ylim = c(0, 1), ylab = "Beta div links", xlab = "Beta div abundance")
```
