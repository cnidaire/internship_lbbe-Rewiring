...
cor_abund_grad <- function(frame = 10, spe = 10){
  ...
  for (i in 1:frame) {
    for (j in i:frame) {
      c <- apply(abind(generated_netw$network[[i]], generated_netw$network[[j]], along = 3), c(1, 2), min)
      beta_div_link[i, j] <- 1 - (2*sum(c) / (sum(generated_netw$network[[i]]) + sum(generated_netw$network[[j]])))
    }
  }
...
```{r}
network <- cor_abund_grad(frame = 60, spe = 10)
plot(network$env_grad, network$abundance, ylim = c(0, 1))
```
