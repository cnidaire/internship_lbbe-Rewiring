...
fig1 <- ggplot() +
  ...
  xlab(TeX("$\\sigma^2$ position correspondance analysis")) +  # Add X axis label
  ylab("delta OS indiv") +  # Add Y axis label
  ggtitle("Consumers") +  # Add plot title
  theme_bw()
...
fig2 <- ggplot() +
  ...
  xlab(TeX("$\\sigma^2$ position correspondance analysis")) +  # Add X axis label
  ylab("delta OS indiv") +  # Add Y axis label
  ggtitle("Resources") +  # Add plot title
  theme_bw()
...
```{r}
figure <- ggplot() +
  geom_point(data = rewiring, aes(x = tot, y = delta_os)) +
  facet_wrap(~spe, scales = "free") +
  xlab("Variance position correspondance analysis") +  # Add X axis label
